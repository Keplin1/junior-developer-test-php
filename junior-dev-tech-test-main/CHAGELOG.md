...
7. Added the aternative colours. Wrapped them in a `<button>` tag, so that they can be selected. Looped through the images and filtered out ones that have broken urls by using `@error`. Added a dynamic class which adds a border around the selected image (`@click`), so that the user can see the colour they chose.
